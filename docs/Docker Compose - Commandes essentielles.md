...
Si le backend échoue avec une erreur PostgreSQL du type `password authentication failed`, vérifier en priorité :

1. que `.env.local` est bien présent sur le serveur ;
2. que `POSTGRES_PASSWORD` correspond au mot de passe attendu par le volume PostgreSQL existant ;
3. que le projet n’exécute pas `python manage.py migrate` automatiquement dans `backend/entrypoint.sh`.

La séquence normale reste :
